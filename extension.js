...
const generate = (componentName, inputPath, mode, enabledOptions) => {
	const basePath = fs.isDirectorySync(inputPath) ? inputPath : path.join(inputPath, '..');
	...
	fs.mkdir(newPath, err => {
		...
		const templatePath =
			vscode.workspace
				.getConfiguration('react-native-component-generator')
		...
		fs.readdir(templatePath, (err, files) => {
			...
			files.map(filename => {
				...
				fs.readFile(filePath, (err, data) => {
					...
					fs.appendFile(
						newFilePath,
						replacePlaceholders(computeConditionals(data.toString(), enabledOptions), componentName),
						() => {}
					);
				});
			});
...
const createDisposable = type =>
	vscode.commands.registerCommand(`extension.generate_${type}`, target => {
...
const activate = context => {
	context.subscriptions.push(createDisposable('rnComponent'));
};
